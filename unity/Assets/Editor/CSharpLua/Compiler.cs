...
//      }
...
#if UNITY_EDITOR_OSX
            string args = " {0}  -s \"{1}\" -d \"{2}\" -l \"{3}\" -m {4} -c";
    ...
    string args = "{csharpLua_}  -s \"{compiledScriptDir_}\" -d \"{outDir_}\" -l \"{lib}\" -m {meta} -c";
      var info = new ProcessStartInfo() {
        ...
        [MenuItem(EditorSettings.kIsRunFromLua
            ? EditorSettings.Menus.kRunFromCSharp
            : EditorSettings.Menus.kRunFromLua)]
